...
/** The barely-visible background population that fills in between the field. */
const DIM_STAR_COUNT = 7000;
...
/**
 * Starfield background on a far sphere: field stars, a dense population of
 * barely-visible ones, and a sparse layer of slightly larger bright ones. The
...
export function createStarfield(): THREE.Group {
  ...
  const layer = (count: number, size: number, brightness: () => number): THREE.Points => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const i3 = i * 3;
      positions[i3] = Math.cos(theta) * r * SKY_RADIUS;
      positions[i3 + 1] = y * SKY_RADIUS;
      positions[i3 + 2] = Math.sin(theta) * r * SKY_RADIUS;
      // Triangular temperature: most stars near white, warm and blue tails.
    ...
    return makeStarPoints(positions, colors, size);
  };

  const group = new THREE.Group();
  // Power-law brightness on the main field: many dim, few bright.
  group.add(layer(SKY_STAR_COUNT, 1.6, () => 0.22 + (SKY_STAR_MAX - 0.22) * Math.pow(rand(), 1.7)));
  // Squared so most of the population hugs the near-invisible floor.
  group.add(layer(DIM_STAR_COUNT, 1.6, () => 0.03 + 0.1 * Math.pow(rand(), 2)));
  group.add(layer(BRIGHT_STAR_COUNT, 2.6, () => 0.4 + (SKY_STAR_MAX - 0.4) * rand()));
  return group;
}
